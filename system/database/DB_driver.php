...
class CI_DB_driver {
	...
	* @return	CI_DB_mysqli_result
	*/
	function multi_query($sql, $binds = FALSE, $return_object = TRUE)
	{
		if ($sql == '')
		{
			if ($this->db_debug)
		...
		if (FALSE === ($this->result_id = $this->conn_id->multi_query($sql)))
		{
			if ($this->save_queries == TRUE)
			{
				$this->query_times[] = 0;
		...
		//penn:check it's a query or a multi_query
		...
		{
			$RES->result_id = $this->conn_id->store_result();
		}
